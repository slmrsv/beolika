      ...
      <div>
        {works?.map((work, index) => (
          <Link key={index} href={`/work/cases/${work.slug}`}>
            <div             
              className={cn('border-b flex items-center justify-between py-6 w-full cursor-pointer hover:text-background hover:bg-primary hover:px-6', 'hover:md:px-8', 'hover:xl:px-10 xl:py-8', 
                {
                  "border-t": index === 0
                }           
              )}>
