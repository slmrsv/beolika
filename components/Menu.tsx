...
const Menu = ({handleClose}: MenuProps) => {
  ...
  return (
    <div className='bg-background h-screen w-full flex flex-col items-center justify-center'>
      <div className={cn('flex flex-col uppercase font-audio')}>
        {navLinks.map((link, index) => (
          <Button
            key={index}
            variant="ghost"
            size="lg"
            className={cn("py-8 w-full text-4xl border-y", "sm:text-5xl sm:py-10", "md:text-5xl md:gap-6", "lg:text-6xl", "xl:text-7xl xl:py-12")}
            asChild
          >
